...
- [ ] 22.2 蒸馏提示词承诺的合并任务:`prompts.py` 写「拿不准时选 ADD —— 多一条重复可以后续再合」,`distiller.py:302` 也说「多一条重复的代价小得多」,但 `register_jobs` 里五个定时任务没有合并,grep 零命中。要么真做(按频道对高相似度的现行条目两两比对),要么把这两处承诺删掉 —— 现在这话是误导,它让 ADD 偏向看起来有兜底而实际是永久累积

- [ ] 22.3 `NullEmbedder` 的静默降级在生产装配里改成显式告警或启动失败:未配置 embedding 凭据时去重与取代对旧记忆基本是瞎的(候选退化成「最近 10 条」),而这件事目前只体现在一行 warning 日志里,影响的却是记忆质量这种要几周才看得出来的东西
