...
- [x] 22. 矛盾记忆在读取侧的裁决依据：`ContextBundle.memory_context` 给每条记忆渲染写入日期，系统提示词给出「说法不一致时以日期较晚的为准」
   - 定性问题:写入侧的去重与取代(蒸馏的 ADD/UPDATE/NOOP)只在语义 top-10 候选内生效,三条路会绕过它 —— 旧记忆没排进候选窗口、向量不可用时候选退化成「最近 10 条」、人工经 Admin API 写入完全不过冲突检查。漏下来的都是两条并列现行而互相矛盾的记忆,而它们相似度几乎相同、会一起进 `top_k`。改造前渲染成裸 `- {content}`,模型手里零信号 —— 这正是 `Design-conversation-context.md` §3.3.1 开头描述的缺陷,而那一节只修了写入侧
   - **日期必须显式写,不能靠顺序暗示**:语义段按相似度排、回落段按时间倒序排,两者渲染出来完全一样,靠位置判断新旧在前一条路上就是错的。测试里刻意把旧的那条放在前面来锁这件事
   - 顺带修掉一个隐蔽 bug:`query_for_context` 为加「偏好(U1): 」前缀而重建 `MemoryEntry`,漏传 `created_at` —— 它有 `default_factory=_utcnow`,于是每条偏好都渲染成「今天」。偏好恰恰最容易前后矛盾(同一个人改了主意,两条都是现行),且不建向量、被全量带上,没有相似度筛选拦着。全标今天等于把裁决依据变成噪声,而这种错不报错、不缺字段,只是模型永远分不出新旧
   - 只给日期不给时刻:矛盾记忆的间隔通常在天到月,精确到秒纯属白耗 token。代价是每条记忆十来个 token
   - 新增 `tests/unit/test_context.py`(6 条)—— `ContextBundle` 此前完全没有测试;`test_prompts.py` 加一条锁裁决规则(日期语义 + 以晚为准 + 否掉顺序暗示三点都要在),`test_memory.py` 加一条锁偏好的 `created_at`

- [x] 22.1 人工写入过冲突检查:`POST /channels/{id}/memories` 写前查近似,命中报 409 带候选,由录入人选「取代第 k 条」还是「并列写入」
   - 定性问题:此前这条路完全不过冲突检查,人在控制台写「超时 5 秒」时库里有蒸馏出的「超时 3 秒」,两条直接并列成为现行,而**没有任何一处会发现**
   - **前置改动:`VectorStore.query` 的返回从 `list[str]` 改成 `list[tuple[str, float]]`** —— 409 的判据是相似度,而此前这个端口只给 id。选择改签名而不是加 `query_scored`:两个近似方法必须返回一致结果,而这个仓库在「两处等价逻辑静默分叉」上有过教训(`should_embed()` 与对账 SQL 谓词那一对)。`_semantic_hits` 显式丢掉分数并注明原因
   - 判据是 `memory_conflict_threshold`(默认 0.85,余弦)。这个数没有先验正确值,取保守初值进配置:宁可少拦(漏掉的还有读取侧日期裁决兜着),动辄拦下不相干的写入会让人学会无脑点「并列写入」,那时这道检查就白设了
   - **不复用 `find_similar`**:那个为蒸馏服务,会把偏好无条件追加进候选(那些条目没有分数,阈值无从施加)、向量不可用时回落到「最近若干条」当候选(原样当冲突等于对每次写入都报一堆不相干的条目)。两种行为在这里都是错的
   - **向量不可用时退化为字面比对**并置 `degraded=true`,控制台明示「只能查字面重复」。默认装配就是 `NullEmbedder`,静默放行等于这道检查在默认部署下没做。字面路径 `score` 给 `null` 不编数,前端显示「字面重复」而非假百分比。⚠️ 归一化刻意不含数字 —— 「超时 3 秒」与「超时 5 秒」的差别全在那个数字上,归一化掉会把真正的矛盾判成字面相同
   - `embed` 返回空(凭据失效)与「没配 embedder」走同一条兜底,否则那条路会静默放行
   - 偏好显式跳过(见 22.4)。跳过不是降级 —— 降级指「本该查却查不了」,两者在 `ConflictCheck` 上是不同状态,测试锁了这个区分
   - `supersede_id` 走 `supersede()` 且 `source=MANUAL`(不是默认的 DISTILLED,那是给蒸馏的)。旧条目不存在或跨频道**合成一条 400** 而不分开报 404/403:两者对调用方是同一件事,分开报会把「B 频道有这个 id」透给 A 频道
   - **前端顺带补一个必须一起做的洞**:这个改动让控制台第一次能发起取代,而控制台此前没有任何地方能看到被取代的条目(`memoryApi.list` 支持 `includeSuperseded`,但 `MemoryPage` 从来没传过)。取代完那条会直接从列表消失且无任何解释 —— 等于加了个结果不可见的操作。故加「含已取代」开关 + 被取代行的「已取代」标记与置灰
   - `ApiError` 加 `body` 字段保留原始响应体(结构化 detail 到不了调用方),`readDetail` 补对象形态的分支(否则后端给的那句人话会掉到兜底文案「请求失败(HTTP 409)」)。类型收窄函数 `readMemoryConflictDetail` 放 types 而非 client —— 后者是全站通用的失败载体,不该认识「记忆冲突」这个领域概念
   - 控制台默认**不预选**任何一项、选出前确认按钮禁用:预选「取代最像的那条」等于替人做了那个判断,而这道界面的全部理由就是不替人做判断
   - `destroyOnHidden` 只在 Modal **关闭**时销毁子节点,切到冲突那一步时 Modal 仍开着,故表单实例与已填内容都活着,「返回修改」原样回来。冲突步的取消是「返回修改」而非关掉 Modal(关掉会把刚写的内容一起丢掉)
   - 冒烟脚本加**组件级用例**(`COMPONENT_CASES`):冲突界面要先撞 409 才出现,页面首帧永远渲不到它。两条(正常 + degraded),因为 degraded 路径 `score` 为 null,`Math.round(null * 100)` 不抛但会渲出「相似度 0%」—— 一个会骗人的显示
   - 新增测试 25 条:`test_admin_memory_conflict.py`(13,真路由 + 真 service + 替身仓储,验的正是两者之间的 409 body 契约)、`test_memory.py` 的冲突段(10)、`test_vector.py` 的 query 形态(2 —— 漏了分数不会在那一层报错,会在上层表现为「阈值怎么调都不起作用」)

- [ ] 22.4 偏好的冲突检查要换机制:偏好按 `should_embed` 不建向量,语义检查对它结构性无效(走向量路径查出零条,会被读成「没有冲突」而真相是「没能力查」)。而偏好恰恰最容易前后矛盾 —— 同一个人改了主意,两条都是现行,且它不建向量、被 `query_for_context` 全量带上,没有相似度筛选拦着。要覆盖只能换机制:字面近似匹配(偏好通常不多,`list_preferences` 本来就是全量无上限),或一次小模型调用

- [x] 22.2a 去掉蒸馏提示词里那句假承诺:原文「拿不准时选 ADD —— 多一条重复可以后续再合」,而那个合并任务从未存在(`register_jobs` 里五个定时任务没有它,grep 零命中)。改成只讲真正的理由 —— 错误取代会作废一条正确的记忆,而多一条重复只是占掉检索名额,两者代价不对等。这个论证本身就足以支撑 ADD 偏向,不需要那个不存在的承诺
   - 顺带更正上一版 tasklist 的一处错判:`distiller.py:302`「多一条重复的代价小得多」**不是**假承诺 —— 它比的是「丢掉这条知识」与「多一条重复」,不涉及合并。那句留着

- [ ]* 22.2b 真要建合并任务前先量:字面重复主要在候选召回失败时形成,而那条路(向量不可用时候选退化成「最近 10 条」)已由 22.3 暴露。建议对账那轮顺便数「频道内高相似度的现行条目对数」,只上报指标不动数据,跑一段看涨不涨 —— 涨才建,不涨说明蒸馏的 NOOP 判断已经够用。届时能自动合的只有**归一化后字面相同**的那部分(内容一样,取代旧的不丢信息);语义相似但措辞不同的不能无人裁决地合(理由同 22.1),那部分要么进人工复核队列(新表 + 新页面,是个真功能)要么不做

- [x] 22.3 把 embedding 降级变可见:未配 embedding 的后果有三层,而此前只有一条启动期 `logger.info` 说了最轻的那层
   - 三层后果(收口成 `_DEGRADED_CONSEQUENCES` 一处文案):① 语义检索关闭,回落到时间倒序;② 手工写入的冲突检查退化为字面比对;③ **蒸馏的去重与取代对旧记忆基本失效** —— 候选退化成「最近 10 条」,三个月前那条矛盾记忆永远进不了比对。第三层最贵且此前一个字都没提
   - `info` → `warning`,且 `embedding_model` 缺失那条分支此前**完全没有日志**(直接 return NullEmbedder),现在两条分支都说
   - 新端点 `GET /api/embedding`(受令牌保护)。**不放 `/health`**:那个端点匿名可打(探针与 `make verify-*` 要用),而「这个部署有没有配 embedding」是运营信息 —— 放进去等于为省一个端点把匿名面扩宽,而 README 刚说了 `/metrics` 暴露运营信息应在反代限制来源,两处取舍该一致。跟 `/tools` 一路
   - 端点带上 `model`:「检索质量怎么变差了」的第一个问题就是配的哪个模型 —— 换过模型而没重建索引时向量是旧模型算的,而那种偏差**对账查不出来**(标记全自洽),要走 `scripts/rebuild_memory_vectors.py`
   - 指标 `teamai_embedder_available`。⚠️ 多进程聚合用 **`min` 而非 `liveall`**:web 与 worker 各报自己那份而两者配置可能不同,`liveall` 取最后写入的会把「有一个进程在降级」抹掉。已加守卫用例锁这个选择 —— 后来者很可能为了「和别的 Gauge 一致」改回去
   - 上报点放 `build_container` 而不是两个 main:两边都经由它起来,埋一处两边自动都有;分别在两个 main 里调,漏掉一个的表现是「看板显示可用,实际那个进程在降级」
   - 记忆页在降级时挂 Alert 列出三层后果。前端单独取这个状态而不并进列表的 `useAsync`:它与频道无关(切频道不必重取),且取失败只该少一条提示、不该让整页进错误态
   - 新增测试 4 条(指标的 min 聚合守卫、端点的可用/不可用/带模型名),并把 `embedder_state` 加进「上报点全都不抛」与「Null 实现全都不抛」两条既有用例
   - 路由表守卫(`test_admin_routes.py` 的 `EXPECTED_ROUTES`)按设计抓到了新端点,已登记
