...
**退避**:`next_attempt_at = now() + min(2^attempts, 300) 秒`,`attempts >= 11` 置 `failed_at`。11 次累计约 20 分钟,足够熬过多数限流窗口;再失败就该有人看告警了。

⚠️ 这里原先写的是「8 次约覆盖 10 分钟」,两处都错:8 次的指数退避累计是 `1+2+4+…+128 = 255` 秒 ≈ 4.2 分钟,而且 300 秒的封顶在 8 次以内永远撞不到(2^7 = 128 < 300),那个上限等于没生效。取 11 让封顶真正起作用(第 9 次起每次都等满 300 秒)。
