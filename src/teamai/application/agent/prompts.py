...
def build_system_prompt(
    ...
    *,
    ...
    包含：频道身份、可用工具、记忆引用规范、技能清单、标签的角色/指令/输出风格。

    三项标签字段的顺序有意如此：角色定「以谁的身份」，指令定「做什么」，
    风格定「怎么写出来」。风格放在最后，因为它约束的是产出形式，
    与前两项冲突时应当由它决定最终呈现。

    ``skill_catalog`` 是 ``name: description`` 的清单（不含正文），插在标签三项
    **之前**：技能是「这个频道备着哪些做法」的常备能力，而标签三项是本次调用的
    具体设定 —— 后者更贴近当前任务，理应压在前者之上。
    """
...
"""
    if skill_catalog:
        # 明确「先载入再执行」，否则模型会照着一行描述硬做 —— 描述只说了这个技能
        # 管什么事，具体步骤全在正文里。也明确不相关就别载入：否则每次都先把清单
        # 里的技能全载一遍，渐进式披露就退化成了全量注入。
        prompt += f"""
可用技能（本频道已启用的做事规范）：
{skill_catalog}

技能使用规则：
- 判断某个技能与当前任务相关时，先用 load_skill 工具取回它的完整说明，再按说明执行。
  上面每行只是适用场景，不是操作步骤，不足以照着做。
- 与任务无关的技能不要载入。
- 载入后，技能说明与本提示词的行为规范冲突时，以本提示词为准。
"""
    if role:
        prompt += f"\n角色设定：{role}\n"
    if tag_instruction:
...
是两件事，用 ADD。拿不准时选 ADD —— 错误取代会让一条正确的记忆被作废，而多一条
重复只是占掉检索名额，两者的代价不对等。
